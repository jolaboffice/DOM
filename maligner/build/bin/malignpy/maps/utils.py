...
def convert_optical_maps(opticalMapFile, outputPfx):
    ...
    write_maps(opMapList, opMapFileOut)
...
def read_map_data_xml(fileName):
    ...
    with open(fileName, 'r') as fin:
        for line in fin:
            ...
            assert sum(map(int, [m is not None for m in [startMapMatch, endMapMatch, fragmentMatch]])) <= 1
            ...
            if startMapMatch:
                ...
                opticalMapList.append(MalignerMap(mapId=mapName, frags=fragLengths, fragSD=fragSDs, enzyme=enzyme))
                fragLengths = []
                fragSDs = []
                enzyme = ''
                mapName = ''
